"""
https://github.com/hemulin/grafana-geo-validators
By hemulin
"""
import os
import pathlib
...
def connect_to_sqlite():
    try:
        conn = sqlite3.connect(DB_DIR / 'validators_geo.db')
...
def insert_validators_geo_data(conn, curr, validators_geo_data, now_timestamp):
    curr.execute('INSERT INTO validators_geo_data VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', (
        str(uuid4()), validators_geo_data['account_address'], validators_geo_data['validator_ip'],
        validators_geo_data['vfn_ip'], validators_geo_data['validator_lat'], validators_geo_data['validator_lon'],
        validators_geo_data['vfn_lat'], validators_geo_data['vfn_lon'], now_timestamp))
    conn.commit()
